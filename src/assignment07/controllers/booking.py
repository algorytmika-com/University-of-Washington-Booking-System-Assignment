...
def get_destination_country() -> str:
    if is_international():
        destination_country = get_input("What is the destination country?:",
                                        'destination_country', "Minimum 5 letters.")
    else:
...
def get_package_description() -> str:
    return get_input("Enter the package description:", 'description',
                     "Minimum 3 letters.")
...
def get_input_weight() -> float:
    return get_input_with_business_validation("weight",
                                              "Enter the weight in kgs [max 10]: ",
                                              'positive_float', POSITIVE_FLOAT)


def get_input_volume() -> float:
    width = get_input_with_business_validation("dimension",
                                               "Enter the width in meters [max 5]: ",
                                               'positive_float', POSITIVE_FLOAT)
    if width == -1:
        return -1
    height = get_input_with_business_validation("dimension",
                                                "Enter the height in meters [max 5]: ",
                                                'positive_float', POSITIVE_FLOAT)
    if height == -1:
        return -1
    depth = get_input_with_business_validation("dimension",
                                               "Enter the depth in meters [max 5]: ",
                                               'positive_float', POSITIVE_FLOAT)
    if depth == -1:
        return -1
...
def get_required_date(is_urgent: bool):
    input = get_input("What is the required delivery date (month/date/year):",
                      'future_date', "Future date (month/date/year)")
    if is_urgent:
        ...
        if d.get_date_converted_from_str(input, '%m/%d/%Y') > d.add_date(days_added=7):
            ...
            print("The package is not urgent - the delivery date cannot be set to less",
...
def is_exit_from_procedure(is_validated) -> bool:
    if is_validated:
        ...
        input = get_input("Do you want to end the booking procedure? [Y/N]:",
                          'true_false', "[Y/N]")
        if input.lower() == 'y':
            return True
        else:
...
def get_input_with_business_validation(business_validation_option, prompt,
                                       input_validation_option, format):
    while True:
...
def get_route(customer_booking : b.Booking) -> r.Route:
    ...
    if (bv.is_heavy(customer_booking) or
            bv.is_large(customer_booking)) and not bv.is_urgent(customer_booking):
