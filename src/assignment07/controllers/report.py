...
def print_shipping_options(booking: pd.DataFrame, title):
    ...
    booking = booking.astype({'air_price': 'string', 'ground_price': 'string',
                              'ocean_price': 'string'})
    ...
    print(booking[['customer_name', 'destination_country', 'package_description',
                   'weight', 'volume', 'is_dangerous', 'is_urgent', 'air_price',
                   'ground_price', 'ocean_price']].to_string(index=False))
